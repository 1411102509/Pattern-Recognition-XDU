...
def K_Nearest_Neighbors(train_Data,test_Data,dataDimension,k=2):
    #数据准备
    # print("*********************************************************************")
...
#                     print(i,"fail to find another position")
                    # break
# 存在自交问题
def cross(population,cross_prob = 0.4):
...
# 变异
def variation(population,variation_prob = 0.1):
    for i in range(len(population)):
...
def GA(dataPath,dataSize,dataDimension,choiceDimension,populationSize,STOP_TIMES):
    ...
    while interation_times < STOP_TIMES:
        ...
        # 变异
        variation(population, variation_prob = 0.4)

    return great_indiv,fit_ave

# 同一维度看收敛情况
if __name__ == "__main__":
    great_indiv,fit_ave = GA("sonar.all-data",208,60,choiceDimension=30,populationSize=10,STOP_TIMES=40)
...
#     Accuracy = []
#     Accuracy.append(0)
#     for dimen in range(1,60):
#         great_indiv,ave = GA("sonar.all-data",208,60,choiceDimension=dimen,populationSize=5,STOP_TIMES=20)
#         Accuracy.append(great_indiv.fitness)
#         plt.scatter(dimen,great_indiv.fitness)
#         Accuracy_AVE += great_indiv.fitness
#         # Accuracy.append(ave[1])
#         # Accuracy_AVE += ave[1]
#     Accuracy_AVE /= 60
