...
def k_Means(dataPath,dataSize,dataDimension,K,testRate=0):
    ...
    for k in range(K):
        ...
        for i in range(len(category[k].clusteringData)):
            if category[k].clusteringData[i][dataDimension] == 'Iris-setosa\n':
                _1_Num += 1
            if category[k].clusteringData[i][dataDimension] == 'Iris-versicolor\n':
                _2_Num += 1
            if category[k].clusteringData[i][dataDimension] == 'Iris-virginica\n':
                _3_Num += 1
        category[k].print()
        ...
        if _1_Num>=_2_Num and _1_Num>=_3_Num:
            ...
            accuracy += _3_Num/len(category[k].clusteringData) * len(category[k].clusteringData)/len(train_Data)
...
def test_K_Means():
    ...
    for i in range(testTimes):
        category,J_temp,accuracy_temp = k_Means("iris.data",150,4,K=3)    #进行K均值聚类,返回聚类结果、误差平方和、精准度
        J.append(J_temp)
    ...
    for k in range(C):
        ...
        for i in range(len(category[k].Data)):
            if category[k].Data[i][dataDimension] == 'Iris-setosa\n':
                _1_Num += 1
            if category[k].Data[i][dataDimension] == 'Iris-versicolor\n':
                _2_Num += 1
            if category[k].Data[i][dataDimension] == 'Iris-virginica\n':
                _3_Num += 1
        category[k].print()
        print("Iris-setosa:    ",_1_Num)
        print("Iris-versicolor:",_2_Num)
        ...
        if _1_Num>=_2_Num and _1_Num>=_3_Num:
            ...
            accuracy += _3_Num/len(category[k].Data) * len(category[k].Data)/len(train_Data)
...
def test_FCM():
    ...
    plt.figure()
    for i in range(testTimes):
        category,accuracy_temp = FCM("iris.data",150,4,C=3,b=2,end_condi=0.001)    #进行K均值聚类,返回聚类结果、误差平方和、精准度
        accuracy.append(accuracy_temp)
